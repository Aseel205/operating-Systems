...
int
wait(uint64 addr  , uint64  messagePointerAddress )
{
  struct proc *pp;
  int havekids, pid;
  struct proc *p = myproc();
...
int waitall(uint64 childsAddress, uint64 childsStauts) { 
  ...
  while (!childAreZombies) {
    ...
    if (!childAreZombies) {
      sleep(p, &wait_lock); //  Avoid busy waiting  , this wait lock is used to safely check and modify the state of child process, to avoid reace conditions 
    }
  }
